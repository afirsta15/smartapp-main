    ...
    <div class="row mt-5 mx-5 pb-5">
      ...
      <div class="col-12 mt-4 px-0 card ">
        ...
        <div v-for="(item, index) in lines" :key="index" class="row m-2">
          ...
          <div class="col">
            <input v-model="item.Harga" class="form-control input-modal" readonly type="text">
          </div>
          <div class="col-1">
...
export default {
  ...
  data () {
    return {
      ...
      date: null,
      description: null,
      store_endpoint: 'item/fetch',
      update_endpoint: 'item/checker',
      lines: []
    }
  },
  created () {
  ...
  methods: {
    ...
    submit () {
      ...
      this.$store.dispatch(this.$store.getters['item/getSelected'] ? this.update_endpoint : this.store_endpoint, params)
        .then((rslt) => {
